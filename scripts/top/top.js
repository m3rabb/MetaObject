...
(function (global) {
	...
	function factory(require) {
		...
		var Top_root              = SpawnFrom(null);
		var 	Stash_root          = SpawnFrom(Top_root);
		var 	Public_root         = SpawnFrom(Top_root);
		var 	Implementation_root = SpawnFrom(Top_root);
		var 		Private_root      = SpawnFrom(Implementation_root);
		var 		Super_root        = SpawnFrom(Implementation_root);

		var Top_root              = SpawnFrom(null);
		var 	Stash_root          = SpawnFrom(Top_root);
		var 	Ref_root            = SpawnFrom(Top_root);
		var 	Implementation_root = SpawnFrom(Top_root);
		var 		Super_root        = SpawnFrom(Implementation_root);
		var 		Thing_root        = SpawnFrom(Implementation_root);
		...
		function IsMethodWithNoPrivateAccess(method) {
			return !POSSIBLE_PRIVATE_ACCESS_MATCHER.test(method.toString());
		}
		...
		function NewUnimplementedMethod(Selector) {
			return AsMethodNamed(function __unimplemented(/* arguments */) {
				return this._noSuchMethod(Selector, arguments);
			}, Selector);
		}

		function NewSuperMethod(Selector) {
			return AsMethodNamed(function (/* arguments */) {  // __super
				var target, targetMethod, parent, currentMethod;

				target = this.__Target;
				targetMethod = target[Selector];
				current = target;

				do {
					current = current.__proto__;
					currentMethod = current[Selector];
				} while (currentMethod === targetMethod);

				return currentMethod.apply(target, arguments);
			}, Selector);
		}
		...
		function ImproperPrivateAccessError() {
			return SignalError(
				this, "Privates method can only be called from within a method!");
			};
		}
		...
		function NewInnerAccessor(Target) {
			return AsMethod(function __target(key, newTarget_) {
		...
		function Cover(inner) {
			var outer = SpawnFrom(inner.__Type._PublicInstanceRoot);
			outer.__inner = NewInnerAccessor(inner);
			inner.__Outer = outer;
			inner._super.__Outer = outer;
			return Lock(outer);
		}
		...
		function EnsureDefaultMethodsFor(selector) {
			if (Implementation_root[selector]) { return; }

			Base_root[selector]  = NewUnimplementedMethod(selector);
			Super_root[selector] = NewSuperMethod(selector);
			Ref_root[selector]   = IsPublicSelector(selector) ?
				NewDelegationMethod(selector) : ImproperPrivateAccessError;
		}

		function InstallMethod(root, selector, method) {
			root[selector] = method;
			EnsureDefaultMethodsFor(selector);
		}

		function InstallMethodSet_(root, selector, baseMethod) {
			var selector$ = selector + "$";
			var method    = AsMethodNamed(baseMethod, selector);
			var method$   = AsMethod$Named(baseMethod, selector$);

			InstallMethod(root, selector , method);
			InstallMethod(root, selector$, method$);
		}

		function InstallMethodSet(root, selector, baseMethod) {
			if (IsValidMethodSelector(selector) && IsAutoGeneratedSelector(selector)) {
				InstallMethodSet_(root, selector, baseMethod);
			}
			SignalError("Selector must be lowecase and not end with $!");
		}
		...
		var 	Stash_root           = SpawnFrom(Top_root);
		var 	_Ref_root  -- __delegationMethod | improperPrivateAccessError
		var 	_Base_root (_Implementation_root|_internal_root)
		var 		_Default_root   -- unimplementedMethod
		var 			Primordial_root -- _noSuchMethod
		var 				Thing_root   -- _noSuchMethod | _respondsTo
		var 				Nothing_root
		var 		_Super_root   -- superMethod
		...
		function NewSuperMethod(Selector) {
			return AsMethodNamed(function (/* arguments */) {  // __super
				...
				return currentMethod ?
					...
					target._noSuchMethod(Selector, arguments);
			}, Selector);
		}
		...
		InstallMethodSet_(Thing_root, "_noSuchMethod", function _noSuchMethod);
		...
		// Used to handle super call to _noSuchMethod, if one should happen!!!
		Base_root._noSuchMethod = AsMethod(function _noSuchMethod(selector, args) {
			return Thing_root._noSuchMethod.call(this, selector, args);
		});

		Type_public = SpawnFrom(Thing_public);
		Type_private = SpawnFrom(Thing_private);
		...
		InstallMethodSet_(publicRoot, privateRoot, selector, baseMethod)
		...
		function SuperMethodOfFor(selector, receiver) {
