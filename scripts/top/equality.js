...
Top.Extend(function () {
  ...
  this.$Thing.Extend(function () {
    this.AddIMethod(function IsEqual(objB, comparator_) {
      if (!objB instanceof Thing) {                    return false; }
      // if (!Top.IsTopObject(objB)) {                    return false; }
    ...
    this.AddIMethod(function _HasEqualAncestry_(_objA, comparator) {
      if (_objA.Type() !== this.Type()) { return false; }
    });
    ...
    this.AddIMethod(function _HasEqualProperties_(_objA, comparator) {
      ...
      if (namesA === namesB) {
        if (namesA) {
          ...
          while (index--) {
            name = namesA[index];
            if (!comparator.AreEqual(_objA[name], _objB[name])) {
              return false;
            }
          }
          return true;
  ...
  function AreEqual(a, b) {
    var isImmutable, comparator, oidA, oidB;

    if (a === b)   {                                      return true;   }
    if (a == null) {                                      return a == b; }
    if (a.IsEqual) {                          return a.IsEqual(b, this); }
  ...
  Top.AddMethod(function IsTopObject(target) {
    return (target instanceof _Implementation);
  });
