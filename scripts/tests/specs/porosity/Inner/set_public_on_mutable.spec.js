HandAxe._(function (
  Thing_, Type_, $BARRIER, $INNER, $OUTER, DeclareImmutable, RootOf
) {
  "use strict"
  ...
  describe("Setting a public property on the inner of mutable object", function() {
    ...
    it("Before setting, it's barrier is not in use", function () {
      expect( this.$inner[$BARRIER].isInUse ).toBe( undefined )
    })
    ...
    describe("When the value is a boolean", function() {
      ...
      it("The receiver's barrier is not in use", function () {
        expect( this.$inner[$BARRIER].isInUse ).toBe( undefined )
      })
